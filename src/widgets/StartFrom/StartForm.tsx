    ...
    return (
        <div className={styles["form__place"]}>
            ...
            <Form form={form} onSubmitCapture={(e: React.FormEvent<HTMLFormElement>) => handleSubmit(e)}>
                <Form.Item className={styles["form__item"]} name="name" label="Имя" rules={[{ required: true, message: '' }]}>
                    <Input placeholder="Елизавета"/>
                </Form.Item>
                <Form.Item className={styles["form__item"]} name="city" label="Город Рождения" rules={[{ required: true, message: '' }]}>
                    <Input placeholder="Санкт-Петербург"/>
                </Form.Item>
                <Form.Item className={styles["form__item"]} name="date" label="Дата Рождения" rules={[{ required: true, message: '' }]}>
                    <DatePicker style={{ width: '100%' }} format="DD.MM.YYYY"/>
                </Form.Item>
                <Form.Item className={styles["form__item"]} name="time" label="Время Рождения" initialValue={dayjs('12:00', format)}>
